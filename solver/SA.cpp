...
std::vector<Param> SA (int niter, int npert,  const std::vector<sunrealtype>& t_list, const  Eigen::Matrix<double, 3*Nstations, Eigen::Dynamic>& data, 
    ...
    double alp    = std::pow((0.000004/0.01), (1.0/niter) );
    ...
    for (int it=0; it<niter ; it++){
        ...
        for (int id_sf=0; id_sf<NSubFaults; id_sf++){
        
            double *pm1= &work.pP[num_sf[id_sf]].k_a_sigma ;
            double *pm2= &work.pP[num_sf[id_sf]].b_a ;
            double *pm3= &work.pP[num_sf[id_sf]].D_c_inv ;
            double *pm4= &work.pP[num_sf[id_sf]].Dtau_asigma ;
            double *pm5= &work.pP[num_sf[id_sf]].V0_ ;
            ...
            for (int j=0; j<nparam ; j++){
                ...
                for (int ipert=0; ipert<npert; ipert++){
                    ...
                    double y = T * std::tan(2*std::acos(0.0) * (alpha - 0.5)) ;
                    // direction
